...
/**
 * SubtitleRetention is duplicated across the two glasses apps with no shared module, exactly
 ...
 */
class SubtitleRetentionDriftTest {

    @Test
    fun `the retention class matches the copy shared with the sibling app`() {
        val expected = javaClass.classLoader
            ?.getResourceAsStream("SubtitleRetention.shared.txt")
            ?.bufferedReader()?.readText()
        assertNotNull("SubtitleRetention.shared.txt is missing from test resources", expected)
        ...
        val actual = run {
            ...
            val f = java.io.File("src/main/java/com/woolab/lumella/SubtitleRetention.kt")
        ...
        assertEquals(
            "SubtitleRetention.kt has drifted from the sibling's copy. If the change is " +
                "intended, apply it to the sibling app and update " +
                "app/src/test/resources/SubtitleRetention.shared.txt in BOTH repos.",
            expected,
            actual,
        )
    }
