...
```bash
ops/take.sh c7 --start --pov   # 걸고 빠진다. 연기를 원하는 만큼 한다
...
- **사진 전에 말로 2~3턴**(40~50초). 그래야 지시자에 `자유대화`가 찍히고,
  사진 턴의 `사진대화`와 대비가 생긴다. 이게 카메라 온디맨드를 보이는 유일한 방법이다
- **3분 안에 끝낼 것.** `screenrecord`의 `--time-limit` 상한이 180초라 그때 화면만
  멈추고 1인칭은 계속 돌아 길이가 어긋난다
