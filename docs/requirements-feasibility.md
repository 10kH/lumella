...
### 확정된 결정 (2026-07-29, 사용자 확인)

| 질문 | 답 | 설계 영향 |
|---|---|---|
| 저장된 표현의 용도 | **메모리 기반 개인화 학습** | 열람 UI가 목적이 아니다 → **로컬 저장으로 충분**. 대신 세션 시작 시 **다시 읽어 페르소나/조종에 주입**하는 경로가 필수다. 저장은 수단이고 목적은 피드백 루프다 |
| 이어폰 효과음 | 적절한 애셋을 골라 사용 | 상태 전환용 **earcon**을 합성해 `res/raw`로 동봉한다. 외부 사운드팩은 라이선스가 얽히고, 상태 큐는 짧은 합성음이 오히려 정석이다 |
| "하루"의 경계 | **기기 로컬 시간대 22:00** | 롤업 시점을 상수로 두되 설정 가능하게. 22:00 이후 대화는 다음 날 버킷 |

> "10시"를 밤 10시로 읽었다. 저녁에 하루를 정리하는 맥락이라 그렇게 잡았고, 상수 하나만 바꾸면 되도록 둔다.

이 결정으로 R4의 성격이 바뀐다. **저장고가 아니라 개인화 입력**이므로:
- 저장 → 다음 세션에서 읽기 → 조종에 주입, 이 왕복이 닫혀야 요구가 충족된다
- R2(주제 다양성)도 같은 저장소를 읽으므로 **하나의 로컬 메모리**로 R2·R4가 동시에 해결된다
