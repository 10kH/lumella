...
| 요구 | 판정 | 진짜 병목 |
|---|---|---|
| R1 핸즈프리 상태 전환 | **구현 완료(2026-08-05)** | 실기기 확인: 연속 청취·VAD 턴·TTFA 482ms·에코 0건 |
| R2 매번 새로운 주제 | ~~부분가능~~ **범위 제외** | 오프너 미채택(2026-07-29 협의) — 사용자가 말을 걸면 시작 |
...
## R1. 핸즈프리 상태 전환 — 구현 완료 (2026-08-05)

착용 없이 실기기에서 확인한 것:

| 확인 항목 | 결과 |
|---|---|
| 세션 READY 시 연속 청취 진입 | `recording=true`, 탭 불필요 |
| VAD가 턴을 열고 닫음 | `음성 감지됨` → `음성 종료됨` → 응답 |
| 응답 지연(TTFA) | 482ms |
| **에코 루프** | **없음.** 조용한 방·스피커 켠 상태에서 튜터가 32.5초 발화하는 동안 VAD 트리거 0건 |

에코 판별은 스피커를 끈 채 30초를 관찰해 방이 조용함을 먼저 확인한 뒤(트리거 0건),
스피커를 켜고 다시 재본 것이다. 그 전 측정에서 나온 트리거는 방에 있던 소리였고
(튜터가 "노래를 따라 하는 것 같네요"라고 응답했다), 튜터 자신의 목소리가 아니었다.

핸즈프리로 바꾸면서 드러난 결함 하나를 같이 고쳤다: 소켓이 응답 도중 끊기면
`responseActive`가 참인 채 남아, 재연결 뒤 모든 턴이 존재하지 않는 응답을 취소하려 들고
서버가 `response_cancel_not_active`를 돌려줘 착용자에게 에러로 보였다.

### 음성 촬영 (2026-08-05 추가)

"사진 찍어서 알려줘"라고 말하면 실제로 찍고, **찍은 것만** 말한다.

| 확인 항목 | 결과 |
|---|---|
| 말로 촬영 요청 → 도구 호출 | `음성 명령: 사진 촬영 (call_id=…)` |
| 카메라 → 모델 | 약 50KB 전송, `sent=true` |
| 응답 지연(사진 턴 TTFA) | 2.5~2.9초 |
| 설명이 실제 프레임과 일치 | 확인 |

**설명이 사실인지 어떻게 확인했나.** 디버그 빌드는 모델에게 보낸 프레임을 그대로
`.../files/model-saw.jpg`에 남긴다. 그걸 꺼내 눈으로 보고 튜터의 말과 맞춰봤다.
어두운 방에서는 "거의 깜깜해서 안 보인다"고 했고 프레임도 실제로 새까맸다.
도형이 있는 그림을 보여주자 "빨간 원, 파란 네모, 초록 세모, 그 위 노란 직사각형,
밑에 갈색 막대"라고 다섯 개를 정확히 짚었다.

이 확인 절차가 필요한 이유는, 고치기 전에 **보지도 않고 지어냈기** 때문이다.
칠흑 같은 방을 두고 "책상 위에 노트북, 커피잔, 노트"라고 자신 있게 말했다.
원인은 셋이었다: 세션에 도구 선언이 0개, 페르소나에 카메라 언급 0건, 그리고
중괄호 하나가 어긋나 `session.update`가 통째로 거부돼(`invalid_json`) 페르소나도
도구도 VAD 설정도 실리지 않은 채 서버 기본값으로 돌고 있었다. 소켓은 열려 있고
화면은 "Ready"라 **아무 이상도 보이지 않았다**.

**같이 고친 것**
- 사진 턴이 9.6초 걸리던 원인: 도구 응답이 단일 스레드 실행기에서 luma 이미지 분석
  (상한 45초) 뒤에 줄 서 있었다. 느린 두뇌는 음성 경로에 올리지 않는다는 원칙을
  조용히 어기고 있었다. 인라인 전송으로 바꾸니 사진 전송→첫 음성이 7788ms → 627ms.
- 조용할 때 탭하면 튜터가 혼잣말하던 것: 판별을 "우리가 바이트를 보냈나"에서
  "서버가 말을 들었나(VAD)"로 교체. 연속 청취라 침묵도 바이트를 쌓아 옛 가드는
  한 번도 발동한 적이 없었다.
- 실수로 앱이 꺼지던 것: 더블탭 종료 앞에 확인 단계를 뒀다.
...
**추가 결정 (2026-07-29)**
- **R2 오프너는 채택하지 않는다.** 에이전트가 먼저 말을 거는 대신, 학습자가 말을 걸면 대화가 시작된다.
  따라서 "오프너 트리거 신설"과 "최근 주제 배제 목록"은 이번 범위에서 빠진다.
  단 주제 다양성 자체는 R3 페르소나에서 "주제는 에이전트가 가져오고 폭넓게 옮겨다닌다"로 흡수했다.
- **연속 스트리밍 비용은 제약이 아니다.** 회사 API를 쓰므로 상시 업로드 구조를 그대로 간다.
  이 문서 앞부분의 "비용 실측 선행" 조건은 해제한다.
