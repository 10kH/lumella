...
# 08/05 수정 요구사항 판정 (2026-08-12)

코드를 직접 확인한 뒤의 판정이다. 근거 파일·라인은 각 항목에 적었다.

## 1) 시야 확보 및 텍스트 가독성

| 항목 | 판정 | 근거 |
|---|---|---|
| 상태 표시 축소·사이드 배치 | **가능** | 현재 48sp 굵은 글씨가 상단 중앙(`activity_main.xml:14-20`, 양 앱 동일). 레이아웃 수정만으로 됨 |
| 발화 텍스트 유지 후 삭제 | **가능** | 현재 학습자가 말을 시작하면(`onSpeechStarted`, `MainActivity.kt:286`) 즉시 지움. 유예 시간 뒤 삭제로 변경 |

## 2) 음성 명령 기능 제어

| 항목 | 판정 | 근거 |
|---|---|---|
| "텍스트 크게/작게/꺼줘/켜줘" | **가능** | 음성 촬영(`capture_photo`)과 같은 function tool 메커니즘. 도구를 세션에 선언하면 모델이 한국어 명령을 알아듣고 호출 |
| "설명 꺼줘/켜줘" | **가능** | 같은 메커니즘, 힌트 줄 토글 |
| "영어/한국어 튜터링 켜줘" | **부분 가능** | 영어(ELLA)와 한국어(lumella)는 별도 앱. 음성 명령으로 **상대 앱을 실행하고 현재 앱을 종료**하는 방식은 가능. 단 대화 맥락은 앱 사이에 이어지지 않음 — 새 세션으로 시작 |

## 3) 한국어 튜터링 개선

| 항목 | 판정 | 근거 |
|---|---|---|
| 응답 간결화 | **가능** | 음성 페르소나(`DEFAULT_SESSION_INSTRUCTIONS`) 수정 |
| recast 제공 | **가능** | 영어 앱에서 검증된 recast-우선 페르소나 패턴 이식 |
| 실행 모델·서비스 표시 | **부분 가능 — 정직한 재구성 필요** | 아래 상세 |

### 모델·서비스 표시의 사실 관계

요구사항은 "응답을 생성하는 모델이 SLM인지 LLM인지"를 전제하지만, 실제 구조가 다르다:

- **들리는 음성은 항상 OpenAI Realtime(LLM)이다.** 빠른 경로가 음성을 만들고, SLM(TANGO)은 느린 경로의 코치로만 참여한다 — 다음 턴의 교수법 조종과 텍스트 근거를 만든다.
- 따라서 **"응답 도중 SLM↔LLM 전환"은 존재하지 않는 현상**이라 표시할 수 없다.
- "말하기평가 SLM-ASE"는 현재 스택에 없는 서비스다.

**가능한 정직한 구현:** luma 턴 응답에 `selectedRoute`/`selectedProvider`가 이미 실려 온다(`schemas/orchestrator.py:145-146`). 이를 힌트 줄 앞에 표시:

```
음성 LLM-GPT · 코치 SLM-TANGO(자유대화)
```

코치 라우트가 바뀌면(자유대화→읽기 등) 다음 턴에서 갱신된다. "실시간"의 단위는 응답 중이 아니라 **턴**이다.

## 07/28 잔여 항목

- **새 주제로 대화 시작(R2)**: 범위 제외 유지 (7/29 협의 — 사용자가 말을 걸면 시작, 주제 다양성은 페르소나에 흡수)
- **하루 학습 표현 저장(R4)**: 부분 가능 판정 유지, 미착수. 별도 작업 규모(로컬 영속 저장소 + 22:00 롤업 + 세션 주입 왕복)라 이번 구현 대상에서 제외

## 이번에 구현하는 것

1. 상태 표시 축소·사이드 + 발화 텍스트 유지 (영어·한국어 양 앱)
2. 음성 명령: 텍스트 크기/표시, 설명 표시, 언어 전환(앱 전환) (양 앱)
3. 한국어 페르소나 간결화 + recast + 모델·서비스 턴 단위 표시 (lumella)
